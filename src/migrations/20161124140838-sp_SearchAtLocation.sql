CREATE PROCEDURE `SearchAtLocation`
    (
      IN orig_lat FLOAT(17,14),
      IN orig_lon FLOAT(17,14),
      IN service_id INT,
      IN street_and_nr VARCHAR(161),
      IN tolerance INT

    )
    BEGIN

    -- Is there a department operational on the nearest address?
    ...
    (SELECT id, 0 as distance, 100 as level,
    concat(street, ' ', housenumber) as `name`,
    'address_match' as `source`
    FROM
      address
    WHERE
      lower(concat(street, ' ', housenumber)) = lower(street_and_nr)) as a JOIN department_address do on a.id = do.address_id
    where do.department_id in (select department_id from department_categories where category_id = 1)
    UNION
    select do.department_id as department_id, a.distance, a.level, a.source from
